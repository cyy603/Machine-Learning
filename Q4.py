...
def add_polymer(list,n):
    ...
    for i in range(n - 1):
        ...
        list = np.column_stack((list, list ** i))
    return list
...
for i in index:
    ...
    if min_error > error:
        ...
        opt_coe = sk_lr.coef_
        min_cost = cost
test_bias = np.multiply(sk_lr.predict(test_X) - test_y, sk_lr.predict(test_X) - test_y)
...
plt.plot(x, sk_lr.predict(test_X), label = 'predict')
plt.legend()
