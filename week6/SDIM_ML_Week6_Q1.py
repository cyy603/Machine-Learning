...
if __name__ == '__main__':
    path= 'E:\chen yuanyao\Machine Learning\SDM\Week6\Homework6\data\Disease\data_1.xlsx'
    train_data = pd.DataFrame(pd.read_excel(path, sheet_name = 'train'))
    test_data = pd.DataFrame(pd.read_excel(path, sheet_name = 'test'))
    train_data.loc[train_data.HYPERTENTION == 0] = -1
    test_data.loc[test_data.HYPERTENTION ==0] = -1
